...
usage() {
	...
	echo ' -n	Dry run mode. Display the COMMAND that would have been executed and exit it.' >&2
	echo ' -s	Execute the COMMAND using sudo on the remote server.' >&2
	echo ' -v	Verbose mode. Displays the server name before executing COMMAND.' >&2
	exit 1
}
# Make sure the sript is not being executed with superuser priviledges
...
#	echo $SSH_COMMAND
	# If it's a dry run, don't execute anything, just cho it.
