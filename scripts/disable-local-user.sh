...
#Remove the optios while leaving the remaining arguments.
shift "$((OPTIND - 1 ))"
...
# Lookp through all the usernames supplied as arguments.
...
do
	...
	then
		...
		then
			...
			if [[ "${?}" -ne 0 ]]
			then
				echo "The archive directory ${ARCHIVE_DIR} could not be craeted."
				exit 1
			fi
		fi
